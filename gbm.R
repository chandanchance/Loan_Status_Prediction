...
gbmFit1 <- train(as.factor(status) ~ ., data = Train[,-1], method = "gbm", trControl = fitControl,verbose = FALSE)

Train$Predicted_status <-predict(gbmFit1,Train[,-1],type= "prob")[,2]
Train$Predicted_status <- ifelse(Train$Predicted_status>=0.5,1,0)
TrainTest$Predicted_status <-predict(gbmFit1,TrainTest[,-1],type= "prob")[,2]
TrainTest$Predicted_status <-as.factor(ifelse(TrainTest$Predicted_status>=0.5,1,0))
Test$Predicted_status <-predict(gbmFit1,Test[,-1],type= "prob")[,2]
Test$Predicted_status <-as.factor(ifelse(Test$Predicted_status>=0.5,1,0))
...
#write.csv(cbind(loan_id,status),"gbm_0.7.csv")
